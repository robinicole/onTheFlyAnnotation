...
class DataToClassifyBase:
    ...
    def sample_data_for_classifier(self) -> pd.DataFrame:
        ...
        sampled_negative_data = negative_data.sample(n_neg_data_to_sample)
        return positive_data.append(sampled_negative_data).sample(frac=1)
...
class DataToClassifyStd(DataToClassifyBase):
    ...
    LDA_PIPELINE: Pipeline = Pipeline(
        [("count", CountVectorizer()), ("lda", LDA(n_components=10, n_jobs=1))]
    )
